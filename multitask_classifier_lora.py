...
from bert_lora import BertModel_LORA
...
class MultitaskBERT(nn.Module):
    ...
    def __init__(self, config, rank):
        ...
        self.bert = BertModel_LORA.from_pretrained('bert-base-uncased', rank=rank)
        ...
        if config.model == 'baseline':
            self.proj_sentiment = torch.nn.Linear(config.hidden_size,
                                                  self.num_labels)  # linear layer for sentiment classification
            self.proj_paraphrase = torch.nn.Linear(config.hidden_size * 2,
                                                   1)  # linear layer for paraphrase classification
            self.proj_similarity = torch.nn.Linear(config.hidden_size * 2,
                                                   1)  # linear layer for similarity classification
...
def train_multitask(args, rank):
    ...
    model = MultitaskBERT(config, rank=rank)
    ...
    print(f"total number of parameters in LoRA BERT: {total_par_num}")
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(lora.lora_state_dict(model), f'./lora_model/model_epoch_{epoch}.pth')
        ...
        sst_dev_acc, sst_dev_y_pred, sst_dev_sent_ids, para_dev_acc, para_dev_y_pred, para_dev_sent_ids, sts_dev_corr, sts_dev_y_pred, sts_dev_sent_ids = model_eval_multitask(
            sst_dev_dataloader, para_dev_dataloader, sts_dev_dataloader, model, device, args.model)
        ...
        if dev_acc > best_dev_acc:
            ...
            save_model(model, optimizer, args, config,
                       args.filepath)  # for other separated (not avg) option, change filepath - diff model for each task
...
if __name__ == "__main__":
    ...
    train_multitask(args, args.rank)
    test_multitask(args, args.rank)
